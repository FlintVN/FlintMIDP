...
public class Sprite extends Layer {
    public static final int TRANS_NONE = 0, TRANS_MIRROR_ROT180 = 1, TRANS_MIRROR = 2,
        TRANS_ROT180 = 3, TRANS_MIRROR_ROT270 = 4, TRANS_ROT90 = 5, TRANS_ROT270 = 6, TRANS_MIRROR_ROT90 = 7;

    private final Image image;
    private final int fw, fh, cols, frameCount;
    private int frame = 0;
    ...
    public Sprite(Image image, int frameWidth, int frameHeight) {
        ...
        this.image = image; this.fw = frameWidth; this.fh = frameHeight;
        this.cols = Math.max(1, image.getWidth() / frameWidth);
        this.frameCount = cols * Math.max(1, image.getHeight() / frameHeight);
    }
    public Sprite(Image image) { this(image, image.getWidth(), image.getHeight()); }
    public Sprite(Sprite s) { this(s.image, s.fw, s.fh); this.frame = s.frame; this.x = s.x; this.y = s.y; }

    public int getRawFrameCount() { return frameCount; }
    public int getFrameSequenceLength() { return sequence != null ? sequence.length : frameCount; }
    ...
    /* minimal collision stubs (return false) */
    ...
    public boolean collidesWith(Image img, int x, int y, boolean pixelLevel) { return false; }
}
