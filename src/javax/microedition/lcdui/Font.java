...
/* Minimal MIDP Font: fixed metrics only. The game uses getHeight/stringWidth/charWidth
 * for layout a lot but drawString just once, so real glyph rendering is deferred. */
public final class Font {
    ...
    private final int height, charW;
    ...
    public static Font getFont(int fontSpecifier) { return DEFAULT; }
    ...
    public int charWidth(char c) { return charW; }
    public int charsWidth(char[] ch, int off, int len) { return len * charW; }
    public int stringWidth(String s) { return (s == null ? 0 : s.length()) * charW; }
    public int substringWidth(String s, int off, int len) { return len * charW; }
}
